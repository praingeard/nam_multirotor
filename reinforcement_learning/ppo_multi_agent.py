...
def main():
    ...
    save_freq=1000,
    save_path="./logs_model/" + str(datetime.now()),
    name_prefix="rl_model_mpc" + str(datetime.now()),
    ...
    env,
    n_eval_episodes=10,
    best_model_save_path="bestmodel_sacmpc_contrchange" + str(datetime.now()),
    log_path="bestmodel_sacmpc_contrchange" + str(datetime.now()),
    eval_freq=500,
    deterministic=False
    )
    ...
    tb_log_name="dqn_airsim_leader_run_new_mpc_contrchange" + str(datetime.now()),
    callback = callbacks,
    progress_bar=True
    )
